...
export function createState() {
  return { turn: 0, board: createTestBoard() };
}

export function createTestBoard() {
  return decodeBoard(boardData7);
}
...
export function decodeBoard(data) {
  ...
  for (let row = 0; row < rows; row++) {
    ...
    for (let column = 0; column < columns; column++) {
      ...
      if (cl == "r" || cl == "b") {
        const player = cl == "b" ? 0 : 1;
        const piece = { player };
        if (c == "R" || c == "B") piece.king = true;
